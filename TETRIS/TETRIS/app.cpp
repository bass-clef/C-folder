...
// ウィンドウの情報
int GetWindowsInfo(UINT uGetInfoType, HWND hWnd){
	int ret = NULL;
	switch(uGetInfoType){
	case GWSI_DESKTOPX:{
		RECT	rc;
...
void DrawRect(HDC hDC, int x1, int y1, int x2, int y2, UINT DrawType){
	...
	case DR_FRAME:{
		RECT	rc = {x1, y1, x2, y2};
		FrameRect(hDC, &rc, (HBRUSH)GetCurrentObject(hDC, OBJ_BRUSH));
		break;}
	case DR_FILL:
	...
	case DR_CFRAME:{
		...
		COLORREF nclr, tclr;
		RECT rc = {x1, y1, x2, y2};
//		nclr = GetDCBrushColor(hDC);
//		tclr = SetDCBrushColor(hDC, RGB(255, 255, 255)-nclr);

		wsprintf(str, "%08X", GetDCBrushColor(hDC));
		Print(hDC, str, 0, 0);

		FrameRect(hDC, &rc, (HBRUSH)GetCurrentObject(hDC, OBJ_BRUSH));
//		SetDCBrushColor(hDC, tclr);
		break;}
	case DR_CFILL:
