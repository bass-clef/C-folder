...
namespace hsp {
	...
	enum {
		OBJ_HWND, OBJ_HDC, OBJ_HINST, OBJ_STYLE
	};
	inline HGDIOBJ objget(int type){
		switch(type){
			case	OBJ_HWND:	return bmscr_stack.p.at(selected_window)->hwnd;
			case	OBJ_HDC:	return bmscr_stack.p.at(selected_window)->hdc;
			case	OBJ_HINST:	return bmscr_stack.p.at(selected_window)->hInst;
			case	OBJ_STYLE:	return (HGDIOBJ)bmscr_stack.p.at(selected_window)->style;
		}
		return NULL;
	}
	inline int ginfo(int type){
		switch(type){
			...
			case  4: { RECT rc; GetWindowRect((HWND)objget(OBJ_HWND), &rc); return rc.left; }
			case  5: { RECT rc; GetWindowRect((HWND)objget(OBJ_HWND), &rc); return rc.top; }
			case  6: { RECT rc; GetWindowRect((HWND)objget(OBJ_HWND), &rc); return rc.right; }
			case  7: { RECT rc; GetWindowRect((HWND)objget(OBJ_HWND), &rc); return rc.bottom; }
	...
	void width(int width = -1, int height = -1, int x = INT_MAX, int y = INT_MAX){
		RECT rc;
		GetClientRect((HWND)objget(OBJ_HWND), &rc);
		if (x != INT_MAX) rc.left = x;
		if (y != INT_MAX) rc.top = y;
		if (width != -1) rc.right = width;
		if (height != -1) rc.bottom = height;
		AdjustWindowRectEx( &rc, (int)objget(OBJ_STYLE),
			GetMenu((HWND)objget(OBJ_HWND)) != NULL ? true : false,
			GetWindowLong((HWND)objget(OBJ_HWND), GWL_EXSTYLE) );
		SetWindowPos((HWND)objget(OBJ_HWND), NULL, rc.left, rc.top, 
	...
	void screen(int wndid = 0, int width = 640, int height = 480, int mode = 0,
		...
		RECT rc;
		GetClientRect(((BMSCR*)p)->hwnd, &rc);
		if (px != INT_MAX) rc.left = px;
		if (py != INT_MAX) rc.top = py;
		if (width != -1) rc.right = width;
		if (height != -1) rc.bottom = height;
		AdjustWindowRectEx(&rc, ((BMSCR*)p)->style, false, 0x100);
		SetWindowPos(((BMSCR*)p)->hwnd, NULL, rc.left, rc.top, 
			rc.right - rc.left, rc.bottom - rc.top,
			SWP_NOACTIVATE | SWP_NOOWNERZORDER | SWP_NOZORDER);
//		hsp::width(width, height);
		ShowWindow(((BMSCR*)p)->hwnd, SW_SHOW);
...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPrev, LPSTR lpCmd, int nCmd){
	hsp::init(hInst, hPrev, lpCmd, nCmd);
	hsp::screen(0);
...
//	ErrorMessageBox();
	hsp::width( 300, 300);
	hsp::tmpstr = "";
	hsp::tmpstr + hsp::ginfo(4) +"x"+ hsp::ginfo(5);
	hsp::tmpstr +"\n"+ hsp::ginfo(6) +"x"+ hsp::ginfo(7);
	hsp::mes( hsp::tmpstr.c_str() );
